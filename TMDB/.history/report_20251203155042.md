...
**實際執行結果**:
- 使用 Random Forest 在驗證集上達到 R² = 0.6884
- RMSE 約為 $72.4M，表示預測誤差約為實際票房的一定比例
- 模型成功識別出 budget 是最重要的特徵（重要性 59.35%）

### 5.2 特徵重要性

**實際 Top 5 重要特徵** (Random Forest):
...
- cast_count: 演員陣容規模
- has_collection: 是否屬於系列電影
- overview_length: 電影簡介長度
- budget_popularity_ratio: 預算與人氣的比例

**關鍵發現**:
- budget 的重要性遠超其他特徵，佔比近 60%
- 數值型特徵（budget, popularity, runtime）合計佔比超過 75%
- 時間特徵（release_month, release_year）也有一定影響
- 文本長度特徵的重要性相對較低
