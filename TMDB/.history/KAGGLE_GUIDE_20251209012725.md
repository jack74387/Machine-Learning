# Kaggle Notebook 使用指南

## 快速開始

### 1. 在 Kaggle 上創建新的 Notebook

1. 前往 [TMDB Box Office Prediction](https://www.kaggle.com/competitions/tmdb-box-office-prediction)
2. 點擊 "Code" → "New Notebook"
3. 選擇 Python 環境

### 2. 複製程式碼

**推薦使用**: `kaggle_notebook.py` （已測試，無語法錯誤）

將 `kaggle_notebook.py` 的內容複製到 Kaggle Notebook 中。

**注意**: 
- 不要使用 `main-2.py`（可能有複製貼上問題）
- 使用 `kaggle_notebook.py` 是最安全的選擇

### 3. 執行程式

直接執行整個 Notebook，程式會自動：
- 載入資料（從 `/kaggle/input/tmdb-box-office-prediction/`）
- 進行特徵工程
- 訓練多個模型
- 生成預測結果（儲存到 `/kaggle/working/submission.csv`）

### 4. 提交結果

1. 點擊右上角的 "Save Version"
2. 選擇 "Save & Run All"
3. 等待執行完成
4. 在 Output 中找到 `submission.csv`
5. 點擊 "Submit to Competition"

---

## 程式特色

### 自動檢測可用套件

程式會自動檢測以下套件是否可用：
- ✅ **XGBoost**: 通常在 Kaggle 預裝
- ✅ **LightGBM**: 通常在 Kaggle 預裝
- ✅ **CatBoost**: 通常在 Kaggle 預裝

如果某個套件不可用，程式會自動跳過該模型。

### 完整的特徵工程

程式包含 **32 個特徵**：

**原始數值特徵 (3)**:
- budget, popularity, runtime

**時間特徵 (5)**:
- release_year, release_month, release_quarter, release_day, release_dayofweek

**計數特徵 (7)**:
- genres_count, cast_count, crew_count, keywords_count
- production_companies_count, production_countries_count, spoken_languages_count

**布林特徵 (3)**:
- has_collection, has_homepage, has_tagline

**文本長度特徵 (4)**:
- title_length, overview_length, tagline_length, original_title_length

**衍生特徵 (4)**:
- budget_popularity_ratio, budget_per_minute
- popularity_per_cast, budget_per_company

**對數特徵 (2)**:
- log_budget, log_popularity

**類別特徵 (3)**:
- original_language, status, top_genre

### 多模型融合

程式會訓練以下模型（如果可用）：
1. **Random Forest** (200 trees, depth=20)
2. **XGBoost** (200 iterations, depth=8, lr=0.05)
3. **LightGBM** (200 iterations, depth=8, lr=0.05)
4. **CatBoost** (200 iterations, depth=8, lr=0.05)

最後使用 **Voting Ensemble** 融合所有模型的預測。

---

## 預期性能

### 驗證集性能

| 模型 | RMSE (預期) | R² (預期) |
|------|-------------|-----------|
| Random Forest | ~$70-75M | ~0.68-0.72 |
| XGBoost | ~$65-70M | ~0.72-0.76 |
| LightGBM | ~$65-70M | ~0.72-0.76 |
| CatBoost | ~$65-70M | ~0.72-0.76 |
| **Voting Ensemble** | ~$60-65M | ~0.75-0.78 |

### Kaggle 排行榜

- **Public Leaderboard**: 預期 RMSE ~$2.5-3.0M (log scale)
- **Private Leaderboard**: 預期類似表現

---

## 進階優化建議

### 1. 超參數調優

使用 GridSearchCV 或 RandomizedSearchCV：

```python
from sklearn.model_selection import RandomizedSearchCV

param_dist = {
    'n_estimators': [100, 200, 300],
    'max_depth': [15, 20, 25],
    'min_samples_split': [2, 5, 10],
    'min_samples_leaf': [1, 2, 4]
}

rf_search = RandomizedSearchCV(
    RandomForestRegressor(random_state=42),
    param_dist,
    n_iter=20,
    cv=5,
    scoring='neg_root_mean_squared_error',
    n_jobs=-1
)
rf_search.fit(X_train, y_train)
```

### 2. 特徵選擇

使用特徵重要性進行選擇：

```python
# 選擇重要性 > 0.01 的特徵
important_features = feature_importance[feature_importance['importance'] > 0.01]['feature'].tolist()
X_train_selected = X_train[important_features]
```

### 3. Stacking Ensemble

實作更進階的 Stacking：

```python
from sklearn.ensemble import StackingRegressor
from sklearn.linear_model import Ridge

stacking_model = StackingRegressor(
    estimators=[
        ('rf', rf_model),
        ('xgb', xgb_model),
        ('lgb', lgb_model)
    ],
    final_estimator=Ridge(),
    cv=5
)
stacking_model.fit(X_train, y_train)
```

### 4. 目標變數轉換

對 revenue 進行 log 轉換：

```python
# 訓練時
y_train_log = np.log1p(y_train)
model.fit(X_train, y_train_log)

# 預測時
predictions_log = model.predict(X_test)
predictions = np.expm1(predictions_log)
```

### 5. 外部資料

考慮加入：
- IMDb 評分
- Rotten Tomatoes 評分
- 社交媒體熱度
- 演員/導演的歷史票房

---

## 常見問題

### Q1: 執行時間太長怎麼辦？

**A**: 減少模型複雜度：
```python
# 減少樹的數量
n_estimators=100  # 從 200 改為 100

# 減少深度
max_depth=15  # 從 20 改為 15
```

### Q2: 記憶體不足怎麼辦？

**A**: 
1. 減少特徵數量（只保留重要特徵）
2. 使用較小的驗證集比例（test_size=0.15）
3. 減少模型數量（只使用 RF + XGBoost）

### Q3: 如何提升排行榜分數？

**A**: 
1. 更多特徵工程（演員/導演特徵）
2. 超參數調優
3. 使用 Stacking 而非 Voting
4. 對目標變數進行 log 轉換
5. 處理異常值

### Q4: 為什麼驗證集 R² 很高但排行榜分數不好？

**A**: 可能是過擬合，建議：
1. 增加正則化（降低 max_depth）
2. 使用交叉驗證
3. 增加訓練資料
4. 減少特徵數量

---

## 檔案結構

```
kaggle-notebook/
├── main-2.py              # Kaggle 版本主程式
├── KAGGLE_GUIDE.md        # 本文件
└── submission.csv         # 生成的預測結果
```

---

## 版本歷史

### v1.0 (main-2.py)
- ✅ 完整的特徵工程（32 個特徵）
- ✅ 多模型支援（RF, XGBoost, LightGBM, CatBoost）
- ✅ Voting Ensemble
- ✅ 自動檢測可用套件
- ✅ Kaggle 路徑適配

---

## 聯絡與支援

如有問題，請參考：
- [Kaggle Discussion](https://www.kaggle.com/competitions/tmdb-box-office-prediction/discussion)
- [專案 README](README.md)
- [完整報告](report.md)

---

**祝你在 Kaggle 競賽中取得好成績！** 🎉
