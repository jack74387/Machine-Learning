# 多類別分類作業報告
## 基於SGDClassifier的MNIST與Fashion MNIST數據集分類研究
...
本研究使用隨機梯度下降分類器(SGDClassifier)對MNIST手寫數字數據集和Fashion MNIST服裝圖像數據集進行多類別分類任務。通過系統性的實驗設計，我們探討了數據增強、正規化處理和超參數調整對分類性能的影響。實驗結果顯示，MNIST數據集的最終測試準確率達到91.47%，Fashion MNIST數據集達到83.11%。研究發現正規化處理是提升性能的關鍵因素，而簡單的像素位移數據增強對這兩個數據集的效果有限。

**關鍵詞：** 機器學習、多類別分類、SGDClassifier、MNIST、Fashion MNIST、數據增強
